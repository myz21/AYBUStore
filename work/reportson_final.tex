...
\author{Süleyman Fatih Sert,Muhammet Yıldız, Utkan Dindaroğlu, Ahmet Selim Yılmaz, \\ Seyfullah Gülyazı, }
...
\begin{center}
    ...
    \captionof{figure}{AYBUStore Enterprise Use Case Diagram }
\end{center}
\end{landscape}
\clearpage
...
This grouping covers the six-use-case minimum (five top-level groups expanding to twelve leaf use cases in total) and explicitly models actor responsibilities so that secondary services (SIS/OTP, Notification, AI Engine) remain decoupled from the primary user-facing flows.
...
\begin{center}
    ...
    \captionof{figure}{System Domain Class Diagram (10 Classes)}
\end{center}

The domain model is divided into two contexts (Domain and Operational) and comprises ten classes that satisfy the SRS in Section~\ref{sec:srs}. Each class lists its key attributes and methods.
...
\begin{itemize}
    ...
    \item \textbf{NotificationService} --- \texttt{notificationId, type, recipient};
    \texttt{sendEmail(uid, subject, body), sendPush(uid, msg), broadcast(msg)}.
    Handles order confirmations, tracking updates, and support-ticket notifications.

    \item \textbf{SupportTicket} --- \texttt{ticketId, userId, issueType, status};
\texttt{createTicket(), assignToAdmin(), resolveTicket()}.
Represents user support requests that may be handled by the AI assistant or escalated to administrators.
\end{itemize}

\noindent\textbf{Key Relationships:}
\begin{itemize}
    \item User \textit{owns} (1:1) Cart; User \textit{places} (1:0..*) Order.
    \item User \textit{creates} (0..*) SupportTicket.
    ...
    \item AIAssistant \textit{recommends} Product and assists SupportTicket resolution.
    \item CMSManager \textit{manages} Product, \textit{administers} Order, and resolves escalated SupportTicket workflows.
    \item NotificationService \textit{notifies} User for confirmations, tracking updates, and support events.
\end{itemize}
